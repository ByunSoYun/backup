...
public class Ex05 {
	public static void main(String[] args) {
		...
		// - 배열 -> 스트림 -> 조작(a) -> 스트림 -> 조작(a) -> 스트림 -> 조작(b) x 반복 
	...
	private static void m8() {
		...
		System.out.println(Data.getIntList().stream().filter(n -> n % 1 ==0).count());
		System.out.println();
	...
	private static void m7() {
		...
		Data.getStringList().stream().map(str->str.length()).sorted().forEach(n -> System.out.println(n));
		System.out.println();
...
//		System.out.println(ulist.get(0));
		...
		ulist.stream()
			...
			//.filter(user->user.getGender() ==1 ) 필터가 올 수 있지만 앞의 객체가 달라서 /파이프-파이프 연결 앞뒤 파이프가 뭐로됑ㅆ어서 반환되는지 확인해야함
			.filter(bmi -> bmi > 25)
	...
	private static void m5() {
		...
		//경우에 따라 파이프의 순서가 결과에 영향을 미치는 경우가 있다.(***주의) 
	...
	private static void m4() {
		...
		System.out.println();
		//중간 파이프는 뒤에 처리를위해 스트림으로 있어야함 -> filter : 스트림(걸러내는작업) 
		...
		//[name=아무개 ,age=28, weight=67,height =172, gender=1]
		...
		Data.getUserList()
				.stream()
				.filter(user-> user.getGender()==1).forEach(user-> System.out.println(user.getName()));
		System.out.println();
		
		//여자만 
		Data.getUserList()
				.stream()
				.filter(user-> user.getGender()==2).forEach(user-> System.out.println(user.getName()));
		System.out.println();
		
		//키 170 over + 몸무게 70 over
		Data.getUserList()
				.stream()
				.filter(user -> user.getHeight()> 170 && user.getWeight() >70).forEach(user -> System.out.println(user));
		System.out.println();
		
		//[name=마우스, size =10, color =RED, data=2019-11-02]
		// [name=모니터 , size=27, color=BLACK, date=2019-11-11]
//		System.out.println(Data.getItemList().size);
...
//		System.out.println(list);
	...
	private static void m1() {
		...
		System.out.println();
		//4. Stream
		// - stream() 메소드 제공 > Stream 탐색 기능 
		// - Arrays.stream(배열)
		// - 컬렉션.stream()
		// - 함수 지향형 프로그래밍 > 메소드 체이닝 형태로 코딩하는것을 지양한다. 
