 ...
 */
 	...
 	// - D:\class\server\JSPtest\src\main\webapp\files (X)
 	// - 톰캣은 프로젝트 소스를 직접 실행하지 않고 복사본 가지고 실행한다.
 	// - 업로드를 저장할 폴더가 톰캣 실행하는 작업 폴더를 지정해야 한다.
 	
 	
 	// "/files" : 가상 경로를 물리 경로로 바꿔준다.
 	
 	//C:\class\server\.metadata\.plugins\org.eclipse.wst.server.core\tmp0\wtpwebapps\JPSTest\files
 	...
 	//*** enctype="multipart/form-data" 으로 선언되면 
 	...
 	try{
 		...
 		MultipartRequest multi = new MultipartRequest(
 									...
 									size, // 업로드 크기 지정 
 									"UTF-8", //인코딩 지정 
 									 new DefaultFileRenamePolicy() // 중복 파일명을 처리 -> 넘버링
 									);
...
<%-- 		
	...
	<h2>다운로드</h2>
		...
		<!-- <a download> 속성이 파일을 무조건 다운로드되게 해준다.  -->
