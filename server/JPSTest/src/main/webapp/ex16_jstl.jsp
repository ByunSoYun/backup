...
<%-- 	
		...
		JSTL 액션 태그, JSP Standard Tag Library
		- <c:XXX> 형식
		- 프로그래밍 기능을 하는 태그
	...
	%>
	<c:set var="c" value="30"  /> <!-- JSPL 변수:  pageContext에 추가되는 변수(속성)-->
	
	<div>a: <%= a %></div>
	<div>a: <%= pageContext.getAttribute("b") %></div>
	<div>a: ${b}</div>
	<%-- <div>c: <%= c %></div> --%>
	<div>${c}</div>   <!--추측? > JSTL 변수가 내장 객체 변수가 아닌지??  -->
