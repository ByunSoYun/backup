...
<body>
	...
	<div class="container">
		...
		<c:if test="${empty id}">
			<button type="button" class="btn btn-default" onclick="alert =" 로그인한
				사용자만 접근이가능합니다."');">회원 전용 페이지1</button>
		</c:if>
		...
		<c:if test="${not empty id}">
			<button type="button" class="btn btn-default"
				onclick="location.href='/jsp/auth/member.do';">회원 전용 페이지2</button>
		</c:if>
		...
		<hr>
			...
			<c:if test="${not empty id}">
			<button type="button" class="btn btn-default"
				onclick="location.href='/jsp/auth/admin.do';">관리자 전용 페이지1</button>
		</c:if>
		...
		<c:if test="${not empty id && lv ==3}">
			<button type="button" class="btn btn-default"
				onclick="location.href='/jsp/auth/admin.do';">관리자 전용 페이지2</button>
		</c:if>
