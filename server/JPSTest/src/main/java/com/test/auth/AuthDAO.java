...
public class AuthDAO {
   ...
   //LoginOk 서브릿이 id와 pw를 줄테니 DB에서 회원이 맞는지를 검사해줘!
   public int login(AuthDTO dto) {
      
      try {
         
         String sql = "select count(*) as cnt from tblUser1 where id=? and pw=? ";
	...
	public AuthDTO login2(AuthDTO dto) {
		...
		try {
			...
			//로그인 처리
			String sql = "select * from tblUser1 where id=? and pw=?";
