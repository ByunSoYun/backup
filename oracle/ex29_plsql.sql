...
begin
    ...
    loop
        ...
        --1.
        select name from tblInsa where num = 1015;
...
create table tblGugudan(
    ...
    constraint tblgugudan_dan_num_pk primary key(dan, num) --복합키1

);

alter table tblGugudan
    constraint tblgugudan_dan_num_pk primary key(dan, num) --복합키2 방식 2개인데 2번을 추천~
...
exception 
    ...
    when others then  -- catch (Exception e) : 모든 종류의 예외처리 
        dbms_output.put_line('예외처리');
end;
...
-- 매개변수 
creaate or replace procedure procTest(pnum number) -- 매개변수 리스트
is 
    vnum number = 0; -- 로컬 변수 
begin 
...
create or replace procedure procTest(
    pbuseo in varchar2, -- 길이지정 x 자료형만
    pname out varchar2
)
is 
...
begin
    ...
    if vcnt > 0 then 
        --위임
        update tblProject set staff_seq = psraff where staff_seq = pseq;
...
/*
1. 저장 프로시저 
    - 반드시 매개변수가 있어야한다 (갯수는 0개 이상, 일반적으로는 1개 이상)
...
--주민번호로 나이대 가져오기 
create or replace fuction fnMaxAnnualIncome(
    p
) return number 
is 
begin
...
-- 반환값을 받는 방식이 차이때문에 사용되는 용도가 다르다.
-- PL/SQL(X)
-- 저장 함수는 ANSI-SQL에서 사용이 가능하다.(***************************)

select name, ``basicpay, sudang ,sudang, fnSum(basicpay,sudang) from tblInsa;


select name, buseo, jikwi, 
    case 
        when substr(ssn,8,1) == '1' then '남자'
        when substr(ssn,8,2) == '2' then '여자'
    end as gender
from tblInsa;
...
is
    vage varchar2(20);
begin
    select (to_char(sysdate,'yyyy') - substr(pbirth,1,4)) into vage from tblStudent;
...
select (to_char(sysdate,'yyyy') - substr(birth,1,4))  from tblStudent;
...
create or replace procedure procTest(
    ...
    pcursor out sys_refcursor   --sys_refcrsor : cursor자료형과 동일한 자료형, 반환값 타입으로 사용     
)
is 
begin
...
declare 
    -- cursor pcursor is select * from tblInsa shere buseo = pbuseo;
    pcursor sys_refcursor;
    vrow tblInsa%rowtype;
begin
    procTest('개발부', pcursor);--(개발부,out참조변수);
...
create table tblLog(
    seq number primary key,
    sseq number not null references tblInsa(num),
...
begin
    ...
    -- 누가?? -> 마지막에 글을 쓴 사람?
    select num from tblBoard where seq = (select max(seq) from tblBoard);
...
--ORA-02292: integrity constraint (HR.SYS_C009016) violated - child record found
-- FK 걸려있어서 안된다
...
-- 그다지 좋은 방법은 아님.
-- 자식이 있는 부모테이블 삭제하는 방법 (부모를 강제로 삭제 , 자식 테이블은 그대로 유지)
...
rollback;
-- delete 하기전엔 꼭 트랜젝션 시작 확인하기!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

create or replace trigger trgInsa
    before 
    delete 
    on tblInsa -- 인사테이블에서삭제 되기 전에 발생
begin
    ...
    if to_char(sysdate, 'dy') = '목' then
        ...
        --현재 실행되려는 작업을 없었던걸로 만들기 -> 강제로 예외 발생!!!(throw new Exception)
        -- - 20000~29999
...
begin 
 ...
 1.생략
    ...
    - 목적 : 행동 자체가 중요한 트리거(적용된 레코드(데이터)는 그다지 중요하지 않음ㅁ
2. 사용 
    - 행 단위 트리거
    - 트리거 실행 횟수 N회
    - DML에 의해서 적용된 행의 갯수와 동일한 횟수만큼 실행
    - 목적 : 적용되는 레코드의 저보가 중요한 트리거
        -가상 컬럼(Pseudo Column)
            a. :old
            b. :new
...
insert into tblBoard values (2, '안녕하세요', default, 'hong'); --글작성
...
create table tblComment (
    ...
    subject varchar2(100) not null,                 --댓긃번호
    regdate date default sysdate not null,         --날짜
...
begin 
    ...
    --댓글 삭제 
    delete from tblComment where pseq = :old.seq; --tblBoard.seq 컬럼값, 삭제될 글 번호
...
begin 
    ...
    --수강신청에서 삭제 
    delete from tblEnrollment e where e.studentSeq = :old.studentSeq;

    
end;


-- 수강신청고유번호 삭제되려면 상담일지/성적/강의평가 에서 삭제되야함
create or replace triger trgDeletteEnrollment
    before
    delete on tblEnrollment
    for each row
begin 
    --상담일지에서 삭제 
    delete from tblCounselingDiary c where c.enrollmentSeq = :old.enrollmentSeq;
    --성적에서 삭제 
    delete from tblScore s where s.enrollmentSeq = :old.enrollmentSeq;
    --강의평가에서 삭제
    delete from tblCourseSurvey css where css.enrollmentSeq = :old.enrollmentSeq;

end;


-- 수강신청고유번호 삭제되려면 성적에서 삭제되야함
create or replace triger trgDeletteEnrollment
    before
    delete on tblEnrollment
    for each row
begin 
    --상담일지에서 삭제 
    delete from tblCounselingDiary where enrollmentSeq =: old.enrollmentSeq;
end;
...
- 오라클은 테이블 생성 시 인덱스를 명시적으로 생성하지 않아도 자동으로 생성한다.
    ...
    2. 일반 컬럼을 where절에 조건으로 select -> 속도 느림
...
select name from tblInssa;  -- 속도 동일
...
set timing on;
-- 인덱스가 없는 상태에 검색
select distinct name from tblIndex where name = '김희예'; --경과 시간: 00:00:01.204
...
--인덱스가 있는 상태에 검색
select distinct name from tblIndex where name = '김희예'; --경과 시간: 00:00:00.011
