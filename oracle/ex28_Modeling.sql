...
ex) 회원 엔티티 > 회원번호(후보키,기본키), 아이디(후보키,대체키), 비밀번호, 주소..
...
- ERD 수정하는 작업(공식)
- 자료의 손실이나, 불필요한 정보를 없애고, 데이터의 일관성을 유지하고, 데이터 종속을 최소화하기 위히
  자료구조를(ERD)를 수정하는 작업
...
데이터베이스 구축 
 -프로젝트 진행 > 다량의 데이터 발생 > 저장하기 위한 조직화된 구조 필요 > 설계 > 그측
 ...
 - 가장 중요한 작업
 - 설계또 작업(***)
 - 요구분석 > 정보(Raw) 수집 > 분석 > 저장 구조 > 도식화(설계화) > ERD(최종 산출물) 
 - 데이터베이스 모델링의 결과물 >ERD
 - 아직 이 단계에서는 DBMS의 종ㅇ류는 결정되지 않는다.
 - 샘플 : ERD
 2. 데이터베이스 설계 
 - 1번의 결과물(ERD) -> 구체화하는 작업
 - 실제 사용할 DBMS를 결정한다. -> 오라클
 ...
 - 샘플 : DDL
 3. 데이터베이스 구축 
 - 1,2 번 결과 토대 > 실체화 > 물리적으로 만드는 작업
 - SQL 사용 
 
 4. 데이터 추가 
 - 90% : 데이터 (유효한 데이터 + 유효하지 않은 데이터)
 - 10% : 정확한 테스트용 데이터 > 업무 확인 
 - 1, 2, 3번 검증 동반 > 데이터 추가할 떄 구조가 올바르지 않으면 문제 발생 
 - 더미 데이터 
 5. 업무 SQL 작성 
 - 요구 분석 > 업무 >실제 SQL
 - 산출물  > 스크립트 (*.sql)
 ...
 - 다른 Entity와 분류(구분)될 수 있고, 다른 Entity에 대해서 정해진 관계를 맺을 수 있는 데이터 단위
 - 객체(실체화 데이터의 집합 - 속성의 집합)
 - 테이블(실체화된 데이터를 담는 역할 컨테이드) - 레코드(실체화된 데이터,행) - 속성(컬럼값,필드,셀)의 집합
 
ex) 회사 정보 관리 프로젝트
    a. 사원 정보 관리 기능 
        - 정보 : 나이, 사원명, 연락처, 주소, 사원번호 등 ..
        - 속성(나이, 사원명, 연락처, 주소, 사원번호 ) 집합을 만듬 -> 사원(엔티티)
    b. 부서 정보 관리 기능 
        - 정보 : 부서명, 부서번호, 사무실번호, 내선번호 등..
        - 속성(부서명, 부서번호, 사무실번호, 내선번호) -> 부서(엔티티)
...
4. 슈퍼키, SUper key
- 복합키, Composite key 
- 1개의 키만으로 식별자 역할이 불가능한 키들 > 2개이상을조합해 기본키 역할 부여 
...
1. 되도록 셀이 null 상태를 가지지 않는다. null 상태의 셀이 많으면 좋지 않다.
2. 중복값을 저장하지 않는다. > 동일한 성격의 데이터 1개 이상의 테이블에 저장하지 않는다.
    > 같은 데이터를 여기저기 동시에 저장하지 않는다 > 공간낭비, 관리 불편
3. 하나의 셀 > 반드시 원자값 저장(단일값, 분리될 수 없는 값(Scalart))   -> 성적 = 100,90,30 이렇게 저장 X 
...
1. 삽입 이상 (Insertion Anormaly) 
    - 특정 테이블에 데이터를 삽입할 떄 원하지 않는 데이터까지 같이 넣어야 하는 상황
...
제 2정규화, 2NF
- 기본키가 아닌 모든 컬럼은 기본키에 완전 함수 종속이여야 한다.
- 부분 함수 종속 컬럼 발견 !! > 부분 함수 종속 제거 
- 일부 컬럼이 복합키 모두에게 종속이 아니라, 복합키 일부에만 종속되는 현상 
