...
public class Ex40_Constructor {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 생성자, Constructor
		 -특수한 목적을 가지는 메소드
		 -객체를 초기화 하는 역할 (객체 멤버 변수를 초기화 or 기타 초기화 과정)
...
//		p2.setAge(-1);
		...
		// p3 : 홍길동, 20
		// p4 : 아무개 , 25
		Person p3 = new Person(); //  익명 , -1 -> 애초에 익명 ,-1로 초기화 될 필요없는데 초기화 후 세터게터로 다시 바꿈 -> 낭비 -> 생성자를 하나 더 만든다 .
		p3.setName("홍길동");
		p3.setAge(20);
		...
		//- ************** 생산자보다 소비자의 입장을 우선한다.
		// - 생산자 : 클래스를 설계하는 개발자 
		// - 소비자: 클래스를 사용해서 객체를 ㅍ생성하는 개발자 
		
		// 생성자 : 생산자를 위한 기술이아니라, 소비자를 위한 기술이다(****)
		
		//생산자가 클래스를 만드는 횟수 : 평생 1번
		//소비자가 클래스를 사용해서 객체를 만드는 횟수 : 0~10000번
		...
		//static -> 사용하기 어렵/ 메모리에 많ㄴ이 상주하기때문에 좋지않다
	...
	}//main -> 종료 -> m도 소멸 ->but new Monitor은 바로 소멸 x garbage 컬렉터가 회수
}
...
class Person{
	...
	//생성자 
	//-메소드 
	//1. 메소드 명이 클래스 명과 동일하다.
	//2. 반환형을 명시하지 않는다. > 반환값을 돌려주는 목적의 메소드가 아니다
	//3. 멤버 초기화를 구현한다.
	//(위의 데이터가 없는 두개의 값에 뭔가를 넣어주는 역할을함)
...
//		this.name = null;
//		this.age = 0;
		// -> 다른 값으로 초기화 하기위해서 적을 뿐 위처럼 초기화 하려고 하면 안적는게 더 편리함
...
/*
  ...
  객체변수의 생명주기
  (static 안붙은거)
  -new, 객체 생성(생성) ~ 객체의 참조가 모두 끊겨서 가비지가 되고,GC가 그 객체를 회수해 가는 순간(소멸)
 
  정적 변수의 생명주기  -> 쓰기도 전에 생성되고 필요없는데도 계속 남아있음 -> 낭비 
