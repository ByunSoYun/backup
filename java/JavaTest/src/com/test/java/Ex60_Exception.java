...
public class Ex60_Exception {
	...
	//main메소드는 예외 미루기를 하면 안된다 -> 더이상 책임져줄 메소드가 없기때문에;;
	// -> 최후의 보루 
	public static void main(String[] args) throws  IOException { //본사 -> 에러발생;;
		...
		// - 나중에 일어날 수 있는 여러가지 예외 상황을 미리 예측해서 그에 따른 대비를 하는 작업 -> 예외처리
	...
	private static void m6() {
		...
		try {
			if(num % 3 != 0) {
				...
				throw e; // 예외 던지기 - 문법적인 오류가 아닌 상화에서 예외를 강제로 발생시킬 떄 사용하는 도구
				//new Exception throw
			}
			System.out.println("업무 진행..");
		} catch (Exception e) {
