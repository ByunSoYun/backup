...
public class Ex69_File {
	...
	private static void m12() {
		...
		//특정 폴더에 들어있는 파일의 갯수 ?
		//- 크기 : 719MB (754,021,291 바이트)
		//- 내용 : 파일 11,901, 폴더 2,600
		String path = "C:\\eclipse";
		File dir = new File(path);
	...
	private static void m8() {
		...
		//폴더 삭제하기
		//- 빈폴더만 삭제가능
		//- 내용물이 있으면 .. 개발자가 그안의 모든 내용물을 각각 직접 삭제한뒤..해당 빈폴더로 만들고 난뒤 삭제한다.
	...
	private static void m7() {
		//폴더명 바꾸기 
		String path = "C:\\class\\java\\file\\DDD"; // 원본
		File dir = new File(path);
		
		String path2 = "C:\\class\\java\\file\\GGG"; // 결과
	...
	private static void m2() {
		...
		if(dir.exists()) {
			...
			System.out.println(dir.length()); // 폴더는 크기가 중요하지않다.
	...
	// - 	C:\class\java\m1.txt
		...
		// 외부 파일 접근 > 파일에 대한 참조객체(대리인) 생성 >참조 객체 조작 >외부 파일 조작
	
		//경로
		String path = "C:\\class\\java\\m1.txt";
		
		//참조 객체 생성
		File file = new File(path);//path는 실제로 있는파일? 없는 파일? -> 상관없이 객체가 만들어진다.
		...
		if(file.exists()) { 
			...
			System.out.println(file.length()); // 18. 파일크기 
			...
			//Tick ->Calendar 
			Calendar c = Calendar.getInstance();
			c.setTimeInMillis(1619500009689L);
